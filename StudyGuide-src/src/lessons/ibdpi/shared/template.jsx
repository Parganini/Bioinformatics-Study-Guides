...
export function IBDPILessonTemplate({ lesson, content, interactions: Interactions, isDone, toggle }) {
  ...
  return (
    <main className="mx-auto w-[min(1120px,calc(100%-24px))] pb-16 pt-8 md:pt-10">
      ...
      <IBDPISection className="mt-8" eyebrow="Slide walkthrough" title="Source-based walkthrough">
                ...
                {group.slides.map((item) => (
                  <div key={`${group.title}-${item.label}-${item.title}`} className="rounded-3xl border border-stone-200 bg-stone-50 p-4">
                    <div className="flex flex-wrap gap-2">
                      <span className="rounded-full border border-stone-200 bg-white px-3 py-1 text-xs font-black text-stone-600">{item.deckLabel}</span>
                      <span className="rounded-full border border-stone-200 bg-white px-3 py-1 text-xs font-black text-stone-600">{item.page}</span>
