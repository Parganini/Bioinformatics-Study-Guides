...
const OPEN_QUESTIONS = [
  {
    ...
    answer: "A GWAS tests genetic variants across the genome to identify markers statistically associated with a phenotype, such as a disease status or a quantitative trait. The goal is to locate genomic regions, candidate genes or biological pathways that may contribute to trait variation.",
  },
  {
    id: "gwas-design",
    topic: "GWAS",
    question: "Describe the main elements that should be considered when designing a GWAS.",
    answer: "A strong GWAS design should include a clear phenotype definition, appropriate genotyping or sequencing data, enough sample size, knowledge of LD structure, control of population structure/stratification, genome-wide significance thresholds with multiple-testing correction, and ideally replication or validation in an independent dataset.",
  },
  {
    id: "manhattan-detail",
    topic: "GWAS",
    question: "Describe a Manhattan plot in detail.",
    answer: "A Manhattan plot displays one point per SNP. The x-axis shows genomic position ordered by chromosome, and the y-axis shows the association strength, usually -log10(P). Peaks indicate genomic regions with strong evidence of association. Horizontal lines often represent suggestive or genome-wide significance thresholds.",
  },
  {
    id: "ld-open",
    topic: "Population genomics",
    question: "What is linkage disequilibrium and why is it important for GWAS?",
    answer: "Linkage disequilibrium is the non-random association between alleles at different loci in a population. It is important because a genotyped SNP can tag nearby causal variants through LD, allowing GWAS to detect associated genomic regions even when the causal variant itself was not directly genotyped.",
  },
  {
    id: "chipseq",
    topic: "Functional genomics",
    question: "What is the primary purpose of ChIP-seq?",
    answer: "ChIP-seq identifies genomic regions bound by a protein of interest, such as a transcription factor or histone modification. Chromatin is immunoprecipitated with an antibody against the protein/mark, and the enriched DNA fragments are sequenced and mapped to the genome.",
  },
  {
    id: "rnaseq",
    topic: "Transcriptomics",
    question: "What is RNA-seq and what biological questions can it answer?",
    answer: "RNA-seq sequences RNA-derived cDNA to measure gene expression, discover transcripts, compare expression between conditions, identify alternative splicing and support genome annotation. A typical workflow includes RNA extraction, library preparation, sequencing, QC, alignment or pseudoalignment, quantification and differential expression analysis.",
  },
  {
    id: "poolseq",
    topic: "Population genomics",
    question: "What is Pool-seq?",
    answer: "Pool-seq is sequencing DNA pools made by combining DNA from multiple individuals. It is cost-effective for estimating allele frequencies in populations, but it usually does not provide individual genotypes and requires careful pooling, coverage and bias control.",
  },
  {
    id: "bisulfite",
    topic: "Epigenomics",
    question: "What is bisulfite sequencing? Briefly describe the strategy.",
    answer: "Bisulfite sequencing is used to study DNA methylation. Bisulfite treatment converts unmethylated cytosines to uracil, which is read as thymine after PCR/sequencing, while methylated cytosines are protected and remain as cytosine. Comparing treated reads to a reference reveals methylated positions.",
  },
  {
    id: "cvalue",
    topic: "Genome size",
    question: "How can genome size be estimated before sequencing using the C-value?",
    answer: "The C-value is the amount of DNA in a haploid genome. It can be measured by cytological or flow-cytometry approaches and converted into base pairs using the approximate relationship 1 pg DNA ≈ 978 Mbp. This gives a prior estimate useful for planning sequencing depth.",
  },
  {
    id: "kmer-genome-size",
    topic: "Genome size",
    ...
    answer: "Reads are decomposed into k-mers and their frequencies are counted. The main genomic peak occurs around the sequencing depth; genome size can be approximated as total k-mers divided by peak depth. Low-frequency k-mers usually represent sequencing errors, the main peak represents true genomic k-mers, and high-frequency k-mers often represent repeats or highly duplicated regions.",
  },
  {
    id: "n50-exercise",
    topic: "Genome assembly",
    question: "Calculate N50 for contig lengths 100, 70, 60, 50, 50, 40 and 30 kb.",
    answer: "Total length = 400 kb, so half is 200 kb. Sort from longest to shortest and calculate the cumulative sum: 100 + 70 = 170, still below 200; adding 60 gives 230, which crosses half of the assembly. Therefore, N50 = 60 kb.",
  },
  {
    id: "coverage-direct",
    topic: "Coverage",
    question: "Estimate the average sequencing depth for a 1 Gbp genome with 100 million reads of 100 bp.",
    answer: "Use depth = (N × L) / G. N = 100,000,000 reads, L = 100 bp, so total sequenced bases = 10,000,000,000 bp. Genome size G = 1,000,000,000 bp. Depth = 10,000,000,000 / 1,000,000,000 = 10×.",
  },
  {
    id: "coverage-inverse",
    topic: "Coverage",
    question: "How many 150 bp reads are required to obtain 30× coverage of a 2.8 Gbp genome?",
    answer: "Use N = (coverage × genome size) / read length. N = (30 × 2.8 × 10^9) / 150 = 84 × 10^9 / 150 = 560,000,000 reads. Therefore, about 560 million 150 bp reads are required.",
  },
  {
    id: "hwe-exercise",
    topic: "Population genetics",
    question: "Estimate whether a population is in HWE with 500 AA, 200 Aa and 300 aa individuals.",
    answer: "Total individuals = 1000 and total alleles = 2000. A alleles = 2×500 + 200 = 1200, so p = 0.6 and q = 0.4. Expected counts are AA = p²×1000 = 360, Aa = 2pq×1000 = 480, and aa = q²×1000 = 160. Observed counts are very different from expected counts, so the population is not in Hardy-Weinberg equilibrium.",
  },
  {
    id: "fastqc-boxplot",
    ...
    answer: "The module shows the distribution of Phred quality scores at each base position, often as boxplots. The median, quartiles and whiskers reveal whether quality drops along the read. Trimming should be considered when low-quality tails, adapters or systematic poor-quality positions are present, especially toward the 3' end.",
  },
  {
    id: "fastq-to-vcf",
    topic: "NGS pipeline",
    question: "Describe a simple NGS variant discovery workflow from FASTQ to VCF.",
    answer: "A typical workflow is: inspect raw FASTQ with FastQC, trim adapters/low-quality bases if needed, align reads to a reference with a mapper such as BWA, sort and index SAM/BAM files, mark/remove duplicates if appropriate, perform variant calling, filter variants, and export SNPs/indels and genotypes in VCF format.",
  },
  {
    id: "busco-open",
    topic: "Genome assembly",
    question: "What is BUSCO and how is it used to evaluate a genome assembly?",
    answer: "BUSCO means Benchmarking Universal Single-Copy Orthologs. It checks whether conserved genes expected to be present as single copies in a lineage are complete, duplicated, fragmented or missing. A high proportion of complete BUSCOs suggests good gene-space completeness.",
  },
  {
    id: "annotation-open",
    topic: "Genome annotation",
    question: "Explain the difference between structural annotation and functional annotation.",
    answer: "Structural annotation identifies genomic features and their coordinates, such as genes, exons, introns, UTRs, repeats and non-coding RNAs. Functional annotation assigns biological meaning to those features, such as gene names, protein domains, pathways, GO terms or putative functions based on similarity and databases.",
  },
  {
    id: "ora-open",
    topic: "Post-GWAS interpretation",
    question: "What is the role of ORA after GWAS?",
    answer: "ORA, or Over-Representation Analysis, is used after candidate gene identification to test whether specific pathways, GO terms or functional categories are enriched more than expected by chance. It helps interpret the biological meaning of GWAS candidate regions.",
  },
  {
    id: "depth-breadth",
    topic: "Coverage",
    question: "What is the difference between depth of coverage and breadth of coverage?",
    answer: "Depth of coverage is the average number of reads covering each base or target position. Breadth of coverage is the proportion of the genome or target region covered at least once or at a specified minimum depth. A sample can have high average depth but poor breadth if coverage is uneven.",
  },
];
