...
import { amlaLessonHref, getAMLANeighbors } from "../amlaManifest.js";

function lessonLabel(lesson) {
  return lesson ? `${lesson.code} ${lesson.title}` : "";
}

export function AMLACanonicalNavigation({ lesson, isDone = false, toggle = () => {}, bottom = false }) {
  const { previous, next } = getAMLANeighbors(lesson?.id);

  return (
    <nav className={`${bottom ? "mt-10" : "mb-6"} rounded-[2rem] border border-stone-200 bg-white/90 p-3 shadow-sm`} aria-label="AMLA lesson navigation">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        {previous ? (
          <a href={amlaLessonHref(previous)} className="rounded-full border border-stone-200 bg-white px-4 py-2 text-sm font-black text-stone-700 transition hover:-translate-y-0.5 hover:shadow-sm">
        ...
        <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
          <a href="#/" className="rounded-full border border-stone-200 bg-white px-4 py-2 text-center text-xs font-black uppercase tracking-[0.18em] text-stone-500 transition hover:bg-stone-50">
            {lesson?.code ? `${lesson.code} - Dashboard` : "AMLA dashboard"}
          </a>
        ...
        {next ? (
          <a href={amlaLessonHref(next)} className="rounded-full bg-stone-950 px-4 py-2 text-sm font-black text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-red-700 hover:shadow-md">
