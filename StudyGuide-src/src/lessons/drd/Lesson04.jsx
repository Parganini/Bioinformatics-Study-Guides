...
function Pill({ children, tone = "red" }) {
  const styles = tone === "teal" ? "border-teal-200 bg-teal-50 text-teal-800" : tone === "stone" ? "border-stone-200 bg-stone-50 text-stone-700" : "border-red-200 bg-red-50 text-red-800";
...
function ResourcePanel({ copy }) {
  const items = [[copy.slides, SLIDES_URL], [copy.transcript, TRANSCRIPT_URL], [copy.recording, CLASS_RECORDING_URL]];
  return <div><div className="text-xs font-black uppercase tracking-[0.18em] text-red-700">{copy.resources}</div><div className="mt-4 grid gap-3">{items.map(([label, url]) => <a key={label} href={url} target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm font-black text-stone-800 transition hover:-translate-y-0.5 hover:bg-white hover:shadow-sm"><span>{label}</span><span className="text-red-700">{copy.open} ↗</span></a>)}</div></div>;
}
function Hero({ copy }) {
  return <section className="overflow-hidden rounded-[2.75rem] border border-stone-200 bg-white/85 shadow-sm"><div className="grid gap-0 lg:grid-cols-[1.25fr_0.75fr]"><div className="p-6 md:p-8"><Pill tone="teal">{copy.heroEyebrow}</Pill><h1 className="mt-5 max-w-4xl text-4xl font-black tracking-tight text-stone-950 md:text-6xl">{copy.heroTitle}</h1><p className="mt-4 max-w-3xl text-lg font-semibold leading-8 text-stone-600">{copy.heroSubtitle}</p><div className="mt-7 flex flex-wrap gap-2">{copy.flow.map(step => <span key={step} className="rounded-full border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs font-black uppercase tracking-[0.14em] text-stone-600">{step}</span>)}</div></div><aside className="border-t border-stone-200 bg-stone-50/70 p-6 lg:border-l lg:border-t-0"><div className="h-full rounded-[2rem] border border-stone-200 bg-white p-5 shadow-inner"><ResourcePanel copy={copy}/><div className="mt-6 rounded-3xl bg-stone-950 p-5 text-white"><div className="text-xs font-black uppercase tracking-[0.18em] text-red-200">Pipeline mindset</div><p className="mt-2 text-lg font-bold leading-7">metadata → object type → inspect → subset → model/report</p></div></div></aside></div></section>;
}
function ExamWatch({ exam, copy }) {
  return <details className="mt-4 rounded-3xl border border-red-200 bg-red-50 p-4"><summary className="cursor-pointer text-sm font-black text-red-800">{copy.exam}: {exam.title}</summary><div className="mt-4 grid gap-3 md:grid-cols-3"><div className="rounded-2xl bg-white p-4"><div className="text-xs font-black uppercase tracking-[0.18em] text-red-700">{copy.include}</div><ul className="mt-3 list-disc space-y-2 pl-5 text-sm font-semibold leading-6 text-stone-700">{exam.include.map(item => <li key={item}>{item}</li>)}</ul></div><div className="rounded-2xl bg-white p-4"><div className="text-xs font-black uppercase tracking-[0.18em] text-red-700">{copy.trap}</div><p className="mt-3 text-sm font-semibold leading-6 text-stone-700">{exam.trap}</p></div><div className="rounded-2xl bg-stone-950 p-4 text-white"><div className="text-xs font-black uppercase tracking-[0.18em] text-red-200">{copy.model}</div><p className="mt-3 text-sm font-semibold leading-6 text-stone-200">{exam.model}</p></div></div></details>;
}
function VisualGrid({ slides, copy, onZoom }) {
  return <div className="mt-6 grid gap-4 md:grid-cols-2">{slides.map((slide, idx) => { const image = IMG[slide.key]; const isOddLast = slides.length % 2 === 1 && idx === slides.length - 1; return <article key={slide.key} className={`overflow-hidden rounded-[2rem] border border-stone-200 bg-stone-50 shadow-sm ${isOddLast ? "md:col-span-2" : ""}`}><button type="button" onClick={() => onZoom({ ...slide, ...image })} className="block w-full border-b border-stone-200 bg-white p-3 text-left"><div className="aspect-[16/10] rounded-2xl bg-stone-100"><img src={image.src} alt={slide.title} loading="lazy" className="h-full w-full rounded-2xl object-contain" /></div></button><div className="p-5"><div className="flex flex-wrap items-center gap-2"><Pill tone="teal">{copy.visualLabel}</Pill><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-stone-500">{copy.zoom}</span></div><h3 className="mt-3 text-xl font-black text-stone-950">{slide.title}</h3><p className="mt-2 text-sm font-semibold leading-6 text-stone-600">{slide.body}</p></div></article>; })}</div>;
}
function LessonSection({ section, copy, onZoom }) {
  return <section className="mt-10 rounded-[2.5rem] border border-stone-200 bg-white/80 p-6 shadow-sm md:p-8"><SectionHeader eyebrow={section.eyebrow} title={section.title}>{section.body}</SectionHeader><div className="rounded-3xl border border-teal-200 bg-teal-50 p-5"><div className="text-xs font-black uppercase tracking-[0.18em] text-teal-800">{copy.professor}</div><p className="mt-2 text-sm font-bold leading-7 text-stone-700">{section.professor}</p></div><ExamWatch exam={section.exam} copy={copy}/><VisualGrid slides={section.slides} copy={copy} onZoom={onZoom}/></section>;
}
...
function ZoomModal({ item, copy, onClose }) {
  if (!item) return null;
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-stone-950/80 p-4" role="dialog" aria-modal="true" onClick={onClose}><div className="max-h-[94vh] w-[min(1100px,96vw)] overflow-hidden rounded-[2rem] bg-white shadow-2xl" onClick={event => event.stopPropagation()}><div className="flex items-center justify-between border-b border-stone-200 p-4"><div><div className="text-xs font-black uppercase tracking-[0.18em] text-red-700">{copy.visualLabel}</div><h3 className="text-xl font-black text-stone-950">{item.title}</h3></div><button onClick={onClose} className="rounded-full bg-stone-950 px-4 py-2 text-sm font-black text-white">{copy.close}</button></div><div className="max-h-[78vh] overflow-auto bg-stone-100 p-4"><img src={item.src} alt={item.title} className="mx-auto max-h-[72vh] w-full object-contain"/></div></div></div>;
}
