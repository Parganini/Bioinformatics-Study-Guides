...
function VariabilityClassifier({ copy }) {
  const [answers, setAnswers] = useState({}); const score = useMemo(() => copy.varItems.filter(item => answers[item.id] === item.type).length, [answers, copy.varItems]);
  return <article className="rounded-[2rem] border border-stone-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-[0.2em] text-red-700">{copy.classEyebrow}</div><h3 className="mt-1 text-2xl font-black text-stone-950">{copy.classTitle}</h3></div><Pill tone={score === copy.varItems.length ? "emerald" : "amber"}>{score}/{copy.varItems.length}</Pill></div><p className="mt-3 text-sm font-semibold leading-7 text-stone-600">{copy.classIntro}</p><div className="mt-5 grid gap-3">{copy.varItems.map(item => { const selected = answers[item.id]; const correct = selected && selected === item.type; const wrong = selected && selected !== item.type; return <div key={item.id} className={`rounded-2xl border p-4 ${correct ? "border-emerald-200 bg-emerald-50" : wrong ? "border-red-200 bg-red-50" : "border-stone-200 bg-stone-50"}`}><div className="flex flex-col justify-between gap-3 md:flex-row md:items-center"><div><div className="text-sm font-black text-stone-950">{item.label}</div>{selected && <div className="mt-1 text-xs font-bold leading-5 text-stone-600">{item.why}</div>}</div><div className="flex gap-2"><button onClick={() => setAnswers({ ...answers, [item.id]: "bio" })} className="rounded-full border border-stone-200 bg-white px-3 py-1.5 text-xs font-black text-stone-700 transition hover:border-red-200 hover:bg-red-50">{copy.bioBtn}</button><button onClick={() => setAnswers({ ...answers, [item.id]: "technical" })} className="rounded-full border border-stone-200 bg-white px-3 py-1.5 text-xs font-black text-stone-700 transition hover:border-red-200 hover:bg-red-50">{copy.techBtn}</button></div></div></div>; })}</div></article>;
}
...
function ExamTrainer({ copy }) { return <section className="mt-10 rounded-[2.5rem] border border-stone-200 bg-white/80 p-6 shadow-sm md:p-8"><SectionHeader eyebrow={copy.examEyebrow} title={copy.examTitle}>{copy.examQuestion}</SectionHeader><div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]"><article className="rounded-3xl border border-red-200 bg-red-50 p-5"><h3 className="text-xl font-black text-red-950">{copy.copySkeleton}</h3><ol className="mt-4 space-y-2 text-sm font-bold leading-6 text-red-900">{copy.skeleton.map(item => <li key={item} className="flex gap-2"><span>•</span><span>{item}</span></li>)}</ol></article><article className="rounded-3xl border border-stone-200 bg-stone-950 p-5 text-white"><h3 className="text-xl font-black">Model answer logic</h3><p className="mt-3 text-sm font-semibold leading-7 text-stone-200">Start with the focused biological question, then define model and variables. Explain that the same measurement always contains both data and variability. Technical variability must be low and known; biological variability is often the phenomenon to interpret. Therefore sample size, replicates, QC and statistics are necessary before drawing conclusions.</p></article></div></section>; }
export default function DRDLesson01({ lang = "es", isDone = false, toggle = () => {} }) {
  ...
  return <main className="mx-auto w-[min(1180px,calc(100%-24px))] pb-16 pt-8 md:pt-12"><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><a href="#/" className="rounded-full border border-stone-200 bg-white px-4 py-2 text-sm font-black text-stone-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">← {copy.back}</a><button onClick={toggle} className={`rounded-full px-4 py-2 text-sm font-black shadow-sm transition hover:-translate-y-0.5 ${isDone ? "bg-emerald-600 text-white" : "bg-stone-950 text-white"}`}>{isDone ? copy.done : copy.mark}</button></div>
    ...
    <QuizBlock copy={copy}/><ExamTrainer copy={copy}/>
    <section className="mt-10 rounded-[2.5rem] border border-stone-200 bg-white/80 p-6 shadow-sm md:p-8"><SectionHeader eyebrow={copy.implEyebrow} title={copy.implTitle}>{copy.implIntro}</SectionHeader><div className="grid gap-4 md:grid-cols-2">{copy.implCards.map(([title, body]) => <article key={title} className="rounded-3xl border border-stone-200 bg-stone-50 p-5"><h3 className="text-xl font-black text-stone-950">{title}</h3><p className="mt-2 text-sm font-semibold leading-6 text-stone-600">{body}</p></article>)}</div></section>
  </main>;
}
