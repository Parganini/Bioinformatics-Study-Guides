...
const sourceDir = process.env.AG_SOURCE_DIR
  ? path.resolve(process.env.AG_SOURCE_DIR)
  : path.join(repoRoot, "tmp", "ag-sources");
...
function firstAvailable(candidates) {
  return candidates.find((candidate) => {
    const probe = spawnSync(candidate, ["-h"], { encoding: "utf8", stdio: "pipe" });
    return probe.status === 0 || probe.status === 99 || /pdftoppm/i.test(`${probe.stdout}${probe.stderr}`);
  });
}

const pdftoppm = process.env.PDFTOPPM || firstAvailable([
  "pdftoppm",
  path.join(process.env.USERPROFILE || "", ".cache", "codex-runtimes", "codex-primary-runtime", "dependencies", "bin", "pdftoppm.cmd"),
]);

const python = process.env.PYTHON || firstAvailable([
  "python3",
  "python",
...
function renderSlide(slide) {
  ...
  run(pdftoppm, ["-f", String(slide.page), "-l", String(slide.page), "-r", "140", "-png", pdfPath, prefix], `Render ${slide.id}`);
...
if (!pdftoppm) {
  throw new Error("Poppler pdftoppm was not found. Install poppler or set PDFTOPPM=/path/to/pdftoppm.");
}

if (!python) {
  throw new Error("Python was not found. Install Python with Pillow or set PYTHON=/path/to/python.");
}
...
if (missing.length) {
  console.error("\nMissing source PDFs. Place these files in tmp/ag-sources or set AG_SOURCE_DIR before rerunning:");
