...
import { agUiCopy, localizeAgSections } from "./agTranslations.js";
    ...
    <>
      <section className="rounded-[2rem] border border-emerald-200 bg-white/90 p-6 shadow-sm md:p-8">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)] lg:items-start">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.32em] text-emerald-700">{labels.appliedGenomics}</p>
            <h1 className="mt-3 text-4xl font-black tracking-tight text-stone-950 md:text-5xl">{labels.guideTitle}</h1>
            <p className="mt-4 max-w-3xl text-lg leading-8 text-stone-700">
              {labels.guideIntro}
            </p>
            {labels.languageNote && (
              <p className="mt-4 max-w-3xl rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold leading-7 text-amber-950">
                {labels.languageNote}
              </p>
            )}
          ...
          <div className="rounded-2xl border border-stone-200 bg-stone-50 p-5">
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.22em] text-stone-500">{labels.guideProgress}</p>
                <p className="mt-2 text-3xl font-black text-stone-950">{progress}%</p>
              </div>
              <p className="text-sm font-bold text-stone-600">{completedCount}/{AG_GUIDE_SECTIONS.length} {labels.sections}</p>
            </div>
      ...
      <section className="mt-6 rounded-[2rem] border border-stone-200 bg-white/90 p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.28em] text-stone-500">{labels.originalSources}</p>
            <p className="mt-2 text-sm font-semibold leading-6 text-stone-600">
              {labels.sourceIntro}
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              <a href={AG_SOURCE_FOLDER} target="_blank" rel="noreferrer" className="rounded-full bg-stone-900 px-3 py-2 text-xs font-black text-white">{labels.driveFolder}</a>
              {AG_TRANSCRIPT_FOLDER ? (
                <a href={AG_TRANSCRIPT_FOLDER} target="_blank" rel="noreferrer" className="rounded-full border border-stone-300 bg-white px-3 py-2 text-xs font-black text-stone-800">{labels.transcripts}</a>
              ) : (
                <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-black text-amber-800">{labels.transcriptsMissing}</span>
              )}
            </div>
          </div>
...
function GuideIndex({ completed, sections, labels }) {
  return (
    <section className="mt-8 rounded-[2rem] border border-stone-200 bg-white/90 p-5 shadow-sm md:p-7">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.28em] text-emerald-700">{labels.pages}</p>
          <h2 className="mt-2 text-3xl font-black text-stone-950">{labels.chronologicalRoute}</h2>
        </div>
        ...
        {sections.map((section) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="rounded-xl bg-white px-3 py-2 text-sm font-black text-emerald-800">{labels.section} {section.number}</div>
              <span className={completed[section.id] ? "rounded-full bg-emerald-700 px-3 py-1 text-xs font-black text-white" : "rounded-full border border-stone-300 bg-white px-3 py-1 text-xs font-black text-stone-600"}>
                {completed[section.id] ? labels.done : labels.open}
              </span>
            </div>
...
function PageNav({ previous, next, labels }) {
  return (
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        {previous && (
          <a href={sectionHref(previous)} className="rounded-full border border-emerald-200 bg-emerald-50 px-5 py-3 text-sm font-black text-emerald-800 transition hover:bg-emerald-100">
            {labels.previous}: {previous.number}
          </a>
        )}
        {next && (
...
export default function AppliedGenomicsGuide({ hash = "", lang = "en" }) {
  ...
  const completedCount = sections.filter((section) => completed[section.id]).length;
  const nextSection = sections.find((section) => !completed[section.id]) || sections[0];
  const progress = percent(completedCount, sections.length);
  const requestedSectionId = sectionIdFromHash(hash);
  const activeSection = sections.find((section) => section.id === requestedSectionId) || null;
  ...
  const followingSection = activeIndex >= 0 && activeIndex < sections.length - 1 ? sections[activeIndex + 1] : null;
  ...
  return (
    <main id="top" className="mx-auto w-[min(1180px,calc(100%-24px))] pb-16 pt-8 md:pt-12">
      <GuideHeader completedCount={completedCount} progress={progress} nextSection={nextSection} sourceLinks={sourceLinks} labels={labels} />

      {activeSection ? (
        <>
          <PageNav previous={previousSection} next={followingSection} labels={labels} />
          <LectureSection
        ...
        <GuideIndex completed={completed} sections={sections} labels={labels} />
      )}

      <section className="mt-8 rounded-[2rem] border border-emerald-200 bg-emerald-50 p-6 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.28em] text-emerald-700">{labels.finalDrill}</p>
            <h2 className="mt-2 text-2xl font-black text-emerald-950">{labels.retrievalTitle}</h2>
            <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-emerald-900">
              {labels.retrievalBody}
            </p>
          </div>
          <a href="#/practice-exam" className="rounded-full bg-emerald-700 px-5 py-3 text-sm font-black text-white shadow-lg shadow-emerald-900/10 transition hover:bg-emerald-800">
            {labels.openPracticeExam}
          </a>
        </div>
      </section>

      <SlideZoom slide={zoomSlide} onClose={() => setZoomSlide(null)} labels={labels} />
    </main>
  );
}
